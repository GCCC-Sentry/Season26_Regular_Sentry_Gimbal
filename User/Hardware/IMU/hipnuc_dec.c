        ...
        case HIPNUC_ID_HI91:
            {
                ...
                static float yaw_accumulator = 0;
                static uint8_t first_frame = 1;
                
                memcpy(&raw->hi91, p + ofs, sizeof(hi91_t));
                
                // 计算yaw累积角度（处理跨越±180°的情况）
                if (!first_frame)
                {
                    float delta_yaw = raw->hi91.yaw - last_yaw;
                    ...
                    yaw_accumulator += delta_yaw;
                }
                else
                {
                    first_frame = 0;
                    yaw_accumulator = raw->hi91.yaw;  // 初始化为当前yaw
                }
                
                last_yaw = raw->hi91.yaw;
                raw->hi91.yaw_cnt = yaw_accumulator;  // 更新累积yaw
